...
Clocking in using the <RedeemLink id={RedeemID.ClockIn} /> redeem will give you [`floor(log2(<clock-ins>))`](https://www.wolframalpha.com/input?i=floor%28log2%28n%29%29+for+1+to+127) Giga VIP.
...
You can only clock in once per stream.

<Aside type="note">
    Clock-ins are reset every season. But don't worry, old data is saved and will be displayed on our websites in the future.
</Aside>
...
Clocking out using the <RedeemLink id={RedeemID.ClockOut} /> redeem will give
you Giga VIP relative to the time you have been clocked-in this stream.

You can only clock out once per stream.

<Aside type="tip">
    Your reward will be higher if you clock out when the credits roll.
</Aside>

### Pyramid Scheme

The <RedeemLink id={RedeemID.PyramidScheme} /> redeem gives out Giga VIP to its redeemers.
...
Giga-vip and other information can be found on [usercards](/usercard/).
